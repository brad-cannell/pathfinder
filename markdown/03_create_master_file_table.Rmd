...
root_dir_file_table <- root_dir_file_table %>% 
  mutate(file_short_name = get_file_name_w_extension(root_dir_file_list))
root_dir_file_table
```

# Left off here. I just made this an r script. Do the same for the other functions below. Want to break this up into a bunch of small functions that work together.
...
```{r}
root_dir_file_table <- root_dir_file_table %>% 
  mutate(file_extension = stringr::str_extract(file_short_name, "(?<=\\.)\\S+"))
root_dir_file_table
```
